...
class Users_Customers extends Users
{
	...
	public function registerCustomers()
	{
		...
		$this->db->set($data2);
		$this->db->insert($this->db->dbprefix . 'Users_Customers');
	...
	public function loginCustomers()
	{
		...
		if ($userId != 0) {
			$query = $this->db->query("select * from Users_Customers where userId=" . $userId . ";");
	...
	{   //TODO: $username must be session data
		$username = $this->db->escape($this->input->post('username'));
		//$query = $this->db->query("select * from Users natural join Users_Customers where username='".$username."';");
		$query = $this->db->query("select * from Users,Users_Customers where username=" . $username . " and Users.id=Users_Customers.userId;");
